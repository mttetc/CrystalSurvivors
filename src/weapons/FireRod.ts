...
export class FireRod extends BaseWeapon {
  ...
  protected fire(_time: number, enemies: Phaser.Physics.Arcade.Group): void {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Wider spread for multi-shot
      let angle = Math.atan2(dy, dx);
      if (count > 1) {
        const spread = 0.25; // radians - wider than standard projectiles
